...
| Claim | Evidence |
|---|---|
| All Terraform ([EKS](terraform/eks.tf "claim:prose"), ECR, S3, KMS, IRSA, GitHub OIDC) is written and validated in CI but has **never been applied to real AWS — with one deliberate exception**: [`scripts/sqs_apply_wizard.sh`](scripts/sqs_apply_wizard.sh "claim:prose") targeted-applied exactly the [SQS queue pair](terraform/sqs.tf "claim:prose") and the [$1 budget alarm](terraform/budget.tf "claim:prose") (#18, 2026-08-21, us-east-1, permanent free tier, alarm created and API-confirmed before the queues). Nothing in this repo has ever provisioned a paid cloud resource. | `terraform/`, `scripts/sqs_apply_wizard.sh`, issue #18 (live-account verification) |
...
| [The SQS queue code carries real visibility-timeout and DLQ semantics](pathfinder/cloud/queue.py "claim:prose") and is tested against emulation. The queue pair is live on real AWS, provisioned by this repo's own Terraform behind a $1 budget alarm (#18) — but **no benchmark has ever run against it**: the live distributed run was skipped by decision (#26, closed unrun), so the LocalStack rehearsal (section 8) is the pipeline evidence and no live-run number exists to quote. | `pathfinder/cloud/queue.py`, `terraform/sqs.tf`, issue #18 |
