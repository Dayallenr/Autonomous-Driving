...
448 tests pass on the Mac; `ruff check` clean.
...
§9 and the rehearsal artifact is checked in. Next: #18 (budget alarm +
targeted Terraform apply of the SQS queues — needs the user's
`aws configure` and a checkpoint first), then #26 (the live run, which
replays §9 with only the config block changed). #27 (demo clip) and the
